...
_StringConst_5:
	.word	39			! length
	.ascii	"Lock not acquired by the current thread"
	.align
_StringConst_4:
	.word	46			! length
	.ascii	"Lock is already acquired by the current thread"
	.align
_StringConst_3:
	.word	51			! length
...
_CheckVersion_P_Synch_:
	...
	set	0x33072ba2,r4		! myHashVal = 856107938
	cmp	r3,r4
	be	_Label_9
	set	_CVMess1,r1
	call	_putString
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if acquired != _P_Thread_currentThread then goto _Label_56		(int)
	load	[r14+8],r1
	load	[r1+4],r1
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if acquired == 0 then goto _Label_61		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	mov	0,r2
	cmp	r1,r2
...
! ASSIGNMENT STATEMENT...
	mov	112,r13		! source line 112
	mov	"\0\0AS",r10
!   acquired = _P_Thread_currentThread		(4 bytes)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	store	r1,[r2+4]
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if acquired == _P_Thread_currentThread then goto _Label_72		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
...
! RETURN STATEMENT...
	...
	mov	"\0\0RE",r10
!   if acquired != _P_Thread_currentThread then goto _Label_90		(int)
	load	[r14+8],r1
	load	[r1+4],r1
	set	_P_Thread_currentThread,r2
	load	[r2],r2
