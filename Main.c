...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    	buffer: array [MAX_BUFFER_SIZE] of char = new array of char {MAX_BUFFER_SIZE of '?'}
    	MIN_BUFFER_SIZE: int = 0
    	bufferNextIn: int = 0
    	bufferNextOut: int = 0
    	semaphore_emptyBuff: Semaphore = new Semaphore						--create a new semaphore for empty buffer
	semaphore_fullbuffer: Semaphore = new Semaphore						--creates a new semaphore for full buffer
	bufferNextIn_lock : Mutex  = new  Mutex							--creates a lock for shared variable bufferNextIn
	bufferNextOut_lock : Mutex  = new  Mutex						--creates a lock for shared variable bufferNextOut
	print_lock    :	Mutex = new Mutex							--creates a lock for printing 								
	thArray: array [8] of Thread = new array of Thread { 8 of new Thread }

  function ProducerConsumer ()

      	buffer = new array of char {MAX_BUFFER_SIZE of '?'}
	bufferNextIn_lock.Init ()								--Initializing Lock of bufferNextIn
	bufferNextOut_lock.Init ()								--Initializing Lock of bufferNextOut
	print_lock.Init() 									--Initializing Lock of print
	semaphore_fullbuffer.Init(MIN_BUFFER_SIZE)						--Initializing the semaphore with empty buffer 
	semaphore_emptyBuff.Init(MAX_BUFFER_SIZE)						--Initializing the semaphore with max buffer size 
	print ("     ")
  ...
  function Producer (myId: int)
	...
	semaphore_emptyBuff.Down()									--Perform Down operation Max_buffer size  
	bufferNextIn_lock.Lock()									--Set Lock for the shared variable bufferNextIn 
	
	print_lock.Lock()										--Sets lock before printing, as value is not changed before printing
        -- Add c to the buffer
        buffer [bufferNextIn] = c									--Inserts character into the buffer
        bufferNextIn = (bufferNextIn + 1) % MAX_BUFFER_SIZE						
        MIN_BUFFER_SIZE = MIN_BUFFER_SIZE + 1								--Increase size of buffer after adding one character

        -- Print a line showing the state
        PrintBuffer (c)											--Prints the character produce by the producer

	print_lock.Unlock()										--Sets unlock for print
	
	semaphore_fullbuffer.Up()									--Perform UP operation MIN_buffer size  
	bufferNextIn_lock.Unlock()									--Unlocks the critical section
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
	semaphore_fullbuffer.Down()									--Perform DOWN operation on MIN_buffer size
	bufferNextOut_lock.Lock()									--Set Lock for the shared variable bufferNextOut 
        
	print_lock.Lock()										--Sets lock before printing, so that the value does not change before printing

	-- Remove next character from the buffer
        c = buffer [bufferNextOut]									--removes a charcter from buffer
        bufferNextOut = (bufferNextOut + 1) % MAX_BUFFER_SIZE							
        MIN_BUFFER_SIZE = MIN_BUFFER_SIZE - 1								--decrements buffer size by 1	
	
        -- Print a line showing the state
        PrintBuffer (c)											--Prints the character consumed by the consumer
	
	print_lock.Unlock()										--Set  Unlock for print
																								
	semaphore_emptyBuff.Up()									--Perform UP operation MAX_buffer size
	bufferNextOut_lock.Unlock()									--Sets unlock for critical section
        -- Perform synchronization...
  ...
  function PrintBuffer (c: char)
    ...
    --
      ...
      j = bufferNextOut
      for i = 1 to MIN_BUFFER_SIZE
        printChar (buffer[j])
        j = (j + 1) % MAX_BUFFER_SIZE
      endFor
      -- Pad out with blanks to make things line up.
      for i = 1 to MAX_BUFFER_SIZE-MIN_BUFFER_SIZE
        printChar (' ')
      endFor
    endFunction
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
	...
	signalEating: array [5] of Condition				-- For each philosopher: storing the condition 
	monitor_lock: Mutex						-- Declaring mutex lock
    methods
      	Init ()
      	PickupForks (p: int)
      	PutDownForks (p: int)
  ...
  behavior ForkMonitor
    ...
    method Init ()
	var i: int
	status = new array of int {5 of THINKING}			--allocating memory for array of status variable			
	signalEating = new array of Condition {5 of new Condition}	--allocating memory for array of Condition variable
	for i = 0 to 4
		signalEating[i].Init()					--Initializing the Condition of each philosopher to Thinking
	endFor
	monitor_lock = new Mutex					--allocating memory for the lock
	monitor_lock.Init()						--Initializing the lock	
      endMethod

    method PickupForks (p: int)
	monitor_lock.Lock()						--Setting up lock for the critical section
	status [p] = HUNGRY						--changing the state of the particular philosopher from Thinking to hungry
	self.PrintAllStatus()						--Printing the changed status
	self.Test(p)							--calls test function, to check if the philosopher can start eating
	while status [p]!= EATING					--checks if the status is not changed to eating 	
		signalEating [p].Wait (& monitor_lock)			--then signals the philosopher to wait	
	endWhile
	monitor_lock.Unlock()						--unlock the critical section
      endMethod

    method PutDownForks (p: int)
 	monitor_lock.Lock()						--Setting the lock for the critical section
	status[p]= THINKING						--Changes the state from Eating to Thinking
	self.PrintAllStatus()						--Prints the state of the particular philosopher	
	self.Test((p+1)%5)						--Calls Test function for the left neighbour of the philosopher
	self.Test((p-1)%5)						--Calls Test function for the right neighbour of the philosopher
	monitor_lock.Unlock()						--Unlocks the critical section
      endMethod

	method Test(p: int)
		while status [p]== HUNGRY && status[(p+1)%5]!= EATING	-- Checks for if the philosopher is hungry and its left and right neighbours are not holding the resources 
		&& status[(p-1)%5]!= EATING
			status[p]= EATING				-- If condition satisfies change the status to Eating			
			self.PrintAllStatus()				--Prints the status of the philosopher
			signalEating[p].Signal (& monitor_lock)		--Signals the other philosophers to get ready
		endWhile
	endMethod  
