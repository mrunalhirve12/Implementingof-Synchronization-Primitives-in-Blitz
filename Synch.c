...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
	waitingThreads = new List[Thread]	--creates a List which hold waiting Threads 
     	endMethod
      ...
      method Lock ()
	var
		 oldIntStat: int						--initializing a variable to store interrupt status
	oldIntStat= SetInterruptsTo(DISABLED)   				--Set the interrupt status to disable
	if acquired == currentThread						--check if  the lock is already  acquired by the  current thread
		FatalError("Lock is already acquired by the current thread")	--Displays error
	endIf 
	--oldIntStat= SetInterruptsTo(DISABLED)
	if !acquired
		acquired = currentThread					--current thread acquires the lock if no thread is acquires lock
	else
		waitingThreads.AddToEnd (currentThread)				--puts the thread in waiting list, since lock is already acquired by other thread
		currentThread.Sleep()						--blocks the current thread
	endIf
	oldIntStat = SetInterruptsTo(oldIntStat)         			--restore the state of Interrupt to Enable
        endMethod
      ...
      method Unlock ()
	var
		oldIntStat: int							--Initialize a variable to hold the status of the interrupt
		t: ptr to Thread						--Initialize a pointer to thread
	
	oldIntStat = SetInterruptsTo (DISABLED)					--Set the interrupt status to disable
	if acquired!= currentThread						--check if the lock is not acquired by the current thread 
		FatalError ("Lock not acquired by the current thread")		--Displays error				
	endIf
	--oldIntStat = SetInterruptsTo (DISABLED)
	t= waitingThreads.Remove()						--Remove a thread from waiting List
	if t									--check if  thread t holds a value
		t.status = READY						--changes the status of the thread from Blocked to Ready
		readyList.AddToEnd(t)						--Moves the thread t in the Ready List
		acquired = t							--Assigning the lock to t thread
	else									--check if waiting list is empty
		acquired = null							--Null value assigned to lock
	endIf
	oldIntStat = SetInterruptsTo (oldIntStat)				--restore the status of the Interrupt to Enable
        endMethod

      ----------  Mutex . IsHeldByCurrentThread  ----------

      method IsHeldByCurrentThread () returns bool
		return acquired == currentThread				--returns true if current thread holds the lock
	endMethod
